"""
GitLab readers utils.

This module contains utility functions for the GitLab readers.
"""
import asyncio
import os
import sys
...
class BufferedGitBlobDataIterator(BufferedAsyncIterator):
    ...
    It is used to retrieve the contents of the files in a GitLab repository.
    ...
    the contents of the files in a GitLab repository.
    ...
    async def _fill_buffer(self) -> None:
        ...
        results: List[GitBlobResponseModel] = await asyncio.gather(
            *[
                self._gitlab_client.get_blob(self._owner, self._repo, blob.sha)
