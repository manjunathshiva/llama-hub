...
Retrieves the contents of a GitLab repository and returns a list of documents.
...
class GitlabRepositoryReader(BaseReader):
    ...
    Examples:
        >>> reader = GitLabRepositoryReader("owner", "repo")
    ...
    def _load_data_from_commit(self, commit_sha: str) -> List[Document]:
        ...
        Loads gitlab repository data from a specific commit sha.
        ...
        commit_response: GitCommitResponseModel = self._loop.run_until_complete(
            self._gitlab_client.get_commit(self._owner, self._repo, commit_sha)
        )
    ...
    def _load_data_from_branch(self, branch: str) -> List[Document]:
        ...
        Loads gitlab repository data from a specific branch.
        ...
        branch_data: GitBranchResponseModel = self._loop.run_until_complete(
            self._gitlab_client.get_branch(self._owner, self._repo, branch)
        )
    ...
    def load_data(
        ...
        Loads gitlab repository data from a specific commit sha or a branch.
    ...
    async def _recurse_tree(
        ...
        (see GitTreeResponseModel.GitTreeObject in
            gitlab_api_client.py for more information)
        ...
        tree_data: GitTreeResponseModel = await self._gitlab_client.get_tree(
    ...
    async def _generate_documents(
        ...
        async for blob_data, full_path in buffered_iterator:
            ...
            url = os.path.join(
                "https://gitlab.com/", self._owner, self._repo, "blob/", id, full_path
            )
            document = Document(
                text=decoded_text,
...
if __name__ == "__main__":
    ...
    reader1 = GitlabRepositoryReader(
        ...
        filter_directories=(
            ["docs"],
            GitlabRepositoryReader.FilterType.INCLUDE,
        ),
    ...
    def load_data_from_commit() -> None:
        ...
        documents = reader1.load_data(
            commit_sha="9000c92213f0e8f8e12b2e0d6cb96b1cd50988bd"
        )
    ...
    input("Press enter to load gitlab repository from branch name...")

    load_data_from_branch()

    # input("Press enter to load gitlab repository from commit sha...")
